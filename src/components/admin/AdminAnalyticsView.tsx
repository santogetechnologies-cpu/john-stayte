...
export function AdminAnalyticsView() {
  ...
  const [dateFilter, setDateFilter] = useState<
    "today" | "7days" | "30days" | "90days" | "year" | "custom"
  >("30days");
  ...
  if (revenueChartData.length >= 2) {
    ...
    const firstHalfAvg =
      revenueChartData.slice(0, mid).reduce((sum, d) => sum + d.revenue, 0) / (mid || 1);
    const secondHalfAvg =
      revenueChartData.slice(mid).reduce((sum, d) => sum + d.revenue, 0) /
      (revenueChartData.length - mid || 1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            Interactive BI charts, order distribution, category performance, and dynamic growth
            trends.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <Link
          to="/admin/orders"
          className="surface-card p-4 rounded-3xl border bg-white space-y-1 shadow-xs block cursor-pointer transition-all hover:border-slate-300 hover:shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="flex items-center justify-between border-b pb-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Daily gross sales graph calculated from recent orders.
              </p>
            </div>
        ...
        <div className="surface-card p-6 rounded-3xl border bg-white space-y-4 shadow-xs">
          <div className="border-b pb-3">
            ...
            <p className="text-xs text-muted-foreground">
              Live breakdown of order fulfillment progression.
            </p>
          </div>
            ...
            <div className="h-64 w-full pt-2">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <RechartsTooltip
                    contentStyle={{ borderRadius: "1rem", border: "1px solid #cbd5e1" }}
                  />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="surface-card p-6 rounded-3xl border bg-white space-y-4 shadow-xs">
          <div className="border-b pb-3 flex items-center justify-between">
            <div>
              <h3 className="font-extrabold text-base text-foreground">
                Catalog Product Stock Levels
              </h3>
              <p className="text-xs text-muted-foreground">
                Real-time inventory levels across active products.
              </p>
            </div>
          ...
          <div className="space-y-2">
            {products.slice(0, 5).map((p) => (
              <div
                key={p.id}
                className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border text-xs"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {p.brand || "Calor"} &bull; {gbp(p.price)}
                    </p>
                  </div>
                </div>
        ...
        <div className="surface-card p-6 rounded-3xl border bg-white space-y-4 shadow-xs">
          <div className="border-b pb-3">
            ...
            <p className="text-xs text-muted-foreground">
              Total inventory units grouped by category.
            </p>
          </div>

          {loading ? (
            <div className="h-64 flex items-center justify-center text-xs text-muted-foreground font-bold gap-2">
              <Loader2 className="h-5 w-5 animate-spin text-primary" /> Loading category
              breakdown...
            </div>
          ) : (
            <div className="h-64 w-full pt-2">
