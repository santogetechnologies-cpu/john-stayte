...
-- Migration: 00085_fix_target_product_ids_text_type.sql
-- Fixes target_product_ids from UUID[] to TEXT[] and removes unsafe UUID casts
-- to support real product string IDs (e.g. prod-mainline-cell, prod-calor-...)
-- ====================================================================

-- 1. CONVERT public.offers.target_product_ids TO TEXT[]
ALTER TABLE public.offers 
  ALTER COLUMN target_product_ids TYPE TEXT[] USING target_product_ids::TEXT[];

ALTER TABLE public.offers 
  ALTER COLUMN target_product_ids SET DEFAULT '{}';

-- 2. UPDATE RECURSIVE CONDITION NODE EVALUATOR WITH SAFE TEXT COMPARISONS
...
BEGIN
  ...
  -- Group node with Operator ("AND" / "OR")
  ...
  IF p_node ? 'type' THEN
    ...
    -- 8. Cart Contains Category (Safe string match)
      ...
      IF p_cart_items IS NOT NULL THEN
        ...
        LOOP
          IF (v_item->>'product_id') IS NOT NULL THEN
            SELECT * INTO v_prod FROM public.products WHERE id::TEXT = (v_item->>'product_id');
    ...
    -- 9. Cart Contains Product (Safe string match)
...
-- 3. UPDATE AUTHORITATIVE VALIDATION & REDEMPTION RPC WITH SAFE TEXT COMPARISONS
...
BEGIN
  ...
  -- G. Calculate Verified Discount from Qualifying Items (Safe string matching)
  ...
  ELSE
    ...
    LOOP
      ...
      v_item_prod_id := COALESCE(v_item->>'product_id', '');
      
      IF v_offer.target_scope = 'category' THEN
        SELECT * INTO v_prod FROM public.products WHERE id::TEXT = v_item_prod_id;
      ...
      ELSIF v_offer.target_scope = 'specific_products' THEN
        IF v_item_prod_id = ANY(v_offer.target_product_ids) THEN
          v_qualifying_subtotal := v_qualifying_subtotal + (v_item_price * v_item_qty);
        END IF;
      END IF;
    END LOOP;
