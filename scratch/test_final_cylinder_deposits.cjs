...
async function testDropdown() {
  const { data: allProducts, error } = await supabase
    .from("products")
    .select("id, name, slug, category_slug, brand, price, specs")
  ...
  const eligible = allProducts.filter(isRefillableLpgCylinderProduct);

  console.log("==================================================================");
  console.log(`TOTAL PRODUCTS IN SUPABASE: ${allProducts.length}`);
  console.log(`TOTAL ELIGIBLE LPG CYLINDER PRODUCTS IN DROPDOWN: ${eligible.length}`);
  console.log("==================================================================");

  eligible.forEach((p, idx) => {
    console.log(`${idx + 1}. [${p.id}] "${p.name}"`);
    console.log(`   Brand: ${p.brand} | Category: ${p.category_slug} | Price: £${p.price}`);
  });
}
