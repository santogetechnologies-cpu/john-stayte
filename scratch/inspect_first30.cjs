...
async function run() {
  ...
  console.log(`Total gas/brand matches: ${gasItems.length}`);
  gasItems.slice(0, 30).forEach(p => {
    console.log(`[${p.id}] ${p.name} | Cat: ${p.category_slug} | Brand: ${p.brand} | Specs: ${JSON.stringify(p.specs)}`);
  });
}
